...
export async function POST(req){
  try{
    ...
    const {bytes,prompt}=await generateBaseImage({product,campaign,slide}); const baseUrl=await saveBytes(`campaigns/${campaign.id}/slide-${slide_index}-base.png`,bytes,'image/png'); const final=await renderHeadline(bytes,{text:slide.text,aspectRatio:campaign.aspect_ratio,position:slide.text_position,fontSize:slide.font_size,textStyle:slide.text_style||'type4',emphasisText:slide.emphasis_text||'',referenceStyleId:campaign.reference_style_id||'auto_ref'}); const finalUrl=await saveBytes(`campaigns/${campaign.id}/slide-${slide_index}-final.png`,final,'image/png');
