...
export async function POST(req){
  try{
    ...
    const csv=['slide,role,text,visual,product_visible',...(campaign.slides||[]).map(s=>[s.slide_index,s.role,s.text,s.visual,s.product_visible].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(','))].join('\n'); zip.file('slide_plan.csv',csv);
