...
function typographyPrompt(slide,campaign){
  const text=String(slide.text||'').trim();
  const emphasis=String(slide.emphasis_text||'').trim();
  const pos=slide.text_position||'top';
  const size=Number(slide.font_size||72);
  const sizeHint=size>=90?'very large':size>=72?'large':size>=56?'medium-large':'medium';
  const styles={
    type1:'elegant editorial serif typography, mostly black, with restrained burgundy/red italic emphasis; clean magazine-like spacing',
    type2:'bold editorial hierarchy: black setup text plus one oversized burgundy/red italic emphasis phrase',
    type3:'creator-style product copy in clean rounded white pill/label shapes with bold black lettering',
    type4:'bold white creator/dialogue lettering with a dark outline for strong readability over the scene',
    type5:'bold compact product-label typography with one strong accent color and minimal copy',
    type6:'large bright-blue italic serif headline with a much smaller black supporting parenthetical/value line',
    type7:'generic modern mobile-chat interface with crisp readable message bubbles and exact speaker labels; no app branding',
    type8:'clear split-comparison typography with PROBLEM and SOLUTION headings, bold readable labels and a red directional arrow'
  };
  const style=styles[slide.text_style]||styles.type4;
  if(!text && !emphasis) return 'Do not add any overlay copy beyond unavoidable real packaging text on the supplied product reference.';
  return `ON-IMAGE COPY IS REQUIRED AND MUST BE LEGIBLE. Render the following exact copy as designed typography, not as tiny caption text. Primary copy EXACTLY: "${text.replace(/"/g,'\\"')}".${emphasis?` Emphasis phrase EXACTLY: "${emphasis.replace(/"/g,'\\"')}".`:''} Typography treatment: ${style}. Place the copy primarily in the ${pos} region with intentional negative space around it. Use ${sizeHint} text sized for a vertical TikTok carousel and make it immediately readable on a phone. Preserve the exact spelling, punctuation, numbers and speaker labels. Do not invent, paraphrase, translate, duplicate, or add extra marketing copy. If a style-reference image contains words, IGNORE those words completely and render ONLY the exact copy supplied here.`;
}
...
export function imagePrompt(product,campaign,slide){
  ...
  const layoutRule=slide.slide_type_id==='N19'?'A deliberate clean 2x2 montage of four macro issue close-ups is REQUIRED for this cover only.':slide.slide_type_id==='N17'?'A deliberate clean vertical split comparison is REQUIRED: problem on left, product/solution on right.':slide.slide_type_id==='N16'?'Create the complete generic mobile-chat style slide, including the embedded casual UGC photo and the exact chat copy specified below. Do not imitate or display any real messaging-app brand name or logo.':'ONE complete scene only. NEVER a collage, contact sheet, before/after grid, or multiple frames.';
  const typography=typographyPrompt(slide,campaign);
  return `${preset.prompt}. Reference style: ${ref.name}. Reference visual DNA: ${ref.imageDNA}. Vertical ${campaign.aspect_ratio} TikTok product-carousel slide. Slide type ${slide.slide_type_id}: ${st.name}. ${st.instruction}. ${layoutRule} Slide role: ${slide.role}. Scene: ${slide.visual}. ${character} ${productRule} ${typography} Keep important subjects and typography away from extreme edges for final ${campaign.aspect_ratio} crop. Preserve generous whitespace where the selected reference style uses it. No third-party logos besides the supplied product branding, no invented prices/retail messaging, no extra watermarks, no levitating products, physics-breaking orientation, warped product scale, moving screens, steam/moving water/fire, or clutter competing with the focal subject.`;
}
...
export async function generateBaseImage({product,campaign,slide}){
  ...
  try{
    ...
    if(styleRef){
      refs.push(styleRef);
      prompt += ' A supplied STYLE REFERENCE image may also be present. Use it only for high-level composition, whitespace, framing, illustration/photo treatment, typography hierarchy and visual hierarchy. Do not copy its person, product, words, labels, logos, exact scene, or distinctive copyrighted character design. Create an original scene for this product. Render only the exact on-image copy specified in the prompt.';
    }
    if(refs.length){
